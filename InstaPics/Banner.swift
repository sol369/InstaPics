...
public class Banner: UIView {
    ...
    private func initializeSubviews() {
        ...
        minimumHeightConstraint = backgroundView.constraintWithAttribute(attribute: .height, .greaterThanOrEqual, to: 80)
        ...
        backgroundView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat(format: "H:|[contentView]|", views: views))
        backgroundView.addConstraint(contentView.constraintWithAttribute(attribute: .bottom, .equal, to: .bottom, of: backgroundView))
        contentTopOffsetConstraint = contentView.constraintWithAttribute(attribute: .top, .equal, to: .top, of: backgroundView)
        ...
        if image == nil {
            ...
            contentView.addConstraint(imageView.constraintWithAttribute(attribute: .leading, .equal, to: contentView, constant: 15.0))
            contentView.addConstraint(imageView.constraintWithAttribute(attribute: .centerY, .equal, to: contentView))
            imageView.addConstraint(imageView.constraintWithAttribute(attribute: .width, .equal, to: 25.0))
            imageView.addConstraint(imageView.constraintWithAttribute(attribute: .height, .equal, to: .width))
            leftConstraintText = "[imageView]"
        }
        let constraintFormat = "H:\(leftConstraintText)-(15)-[labelView]-(8)-|"
        contentView.translatesAutoresizingMaskIntoConstraints = false
        contentView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat(format: constraintFormat, views: views))
        contentView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat(format: "V:|-(>=1)-[labelView]-(>=1)-|", views: views))
        backgroundView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat(format: "H:|[contentView]-(<=1)-[labelView]", options: .alignAllCenterY, views: views))
        
        for view in [titleLabel, detailLabel] {
            let constraintFormat = "H:|[label]-(8)-|"
            contentView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat(format: constraintFormat, options: .directionLeadingToTrailing, metrics: nil, views: ["label": view]))
        }
        labelView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat(format: "V:|-(10)-[titleLabel][detailLabel]-(10)-|", views: views))
    }
    ...
    override public func didMoveToSuperview() {
        ...
        commonConstraints = self.constraintsWithAttributes(attributes: [.leading, .trailing], .equal, to: superview)
        superview.addConstraints(commonConstraints)
        showingConstraint = self.constraintWithAttribute(attribute: .top, .equal, to: .top, of: superview)
        let yOffset: CGFloat = -7.0 // Offset the bottom constraint to make room for the shadow to animate off screen.
        hiddenConstraint = self.constraintWithAttribute(attribute: .bottom, .equal, to: .top, of: superview, constant: yOffset)
    }
    ...
    public func show(view: UIView? = Banner.topWindow(), duration: TimeInterval? = nil) {
...
extension UIView {
    func constraintsEqualToSuperview(edgeInsets: UIEdgeInsets = UIEdgeInsets.zero) -> [NSLayoutConstraint] {
        self.translatesAutoresizingMaskIntoConstraints = false
        var constraints = [NSLayoutConstraint]()
        if let superview = self.superview {
            constraints.append(self.constraintWithAttribute(attribute: .leading, .equal, to: superview, constant: edgeInsets.left))
            constraints.append(self.constraintWithAttribute(attribute: .trailing, .equal, to: superview, constant: edgeInsets.right))
            constraints.append(self.constraintWithAttribute(attribute: .top, .equal, to: superview, constant: edgeInsets.top))
            constraints.append(self.constraintWithAttribute(attribute: .bottom, .equal, to: superview, constant: edgeInsets.bottom))
        }
        return constraints
    }
    ...
    func constraintsWithAttributes(attributes: [NSLayoutAttribute], _ relation: NSLayoutRelation, to item: AnyObject, multiplier: CGFloat = 1.0, constant: CGFloat = 0.0) -> [NSLayoutConstraint] {
        return attributes.map { self.constraintWithAttribute(attribute: $0, relation, to: item, multiplier: multiplier, constant: constant) }
    }
}
